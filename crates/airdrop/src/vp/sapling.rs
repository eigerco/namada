    ...
    CTX: VpEnv<'ctx> + namada_tx::action::Read<Err = Error>,
{
    ...
    let note_commitment_root_bytes: [u8; 32] = ctx
        .read_bytes_post(&sapling_key::note_commitment_root_key())?
    ...
    let nullifier_gap_root_bytes: [u8; 32] = ctx
        .read_bytes_post(&sapling_key::nullifier_gap_root_key())?
    ...
    let target_id: Vec<u8> = ctx
        .read_bytes_post(&sapling_key::target_id_key())?
    ...
    let scheme_id: u8 = ctx
        .read_bytes_post(&sapling_key::value_commitment_scheme_key())?
